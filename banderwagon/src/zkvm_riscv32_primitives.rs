...
#[cfg(feature = "zkvm-riscv32-sim")]
fn u32_array_to_biguint_le(input: &[u32; 8]) -> BigUint {
...
#[cfg(feature = "zkvm-riscv32-sim")]
fn biguint_to_u32_array_le(input: &BigUint) -> [u32; 8] {
    let mut bytes = [0u8; 32];
    let input_bytes = input.to_bytes_le();
    bytes[..input_bytes.len().min(32)].copy_from_slice(&input_bytes[..input_bytes.len().min(32)]);

    let mut output = [0u32; 8];
    for i in 0..8 {
        output[i] = u32::from_le_bytes(bytes[i * 4..(i + 1) * 4].try_into().unwrap());
    }
    output
}
...
#[cfg(zkvm_riscv32)]
#[inline(always)]
fn backend_mul_mod_u32(result: &mut [u32; 8], x: &[u32; 8], y: &[u32; 8], modulus: &[u32; 8]) {
    unsafe {
        sys_bigint(result, OP_MULTIPLY, x, y, modulus);
    }
}
...
#[inline(always)]
fn backend_mul_mod_u32(result: &mut [u32; 8], x: &[u32; 8], y: &[u32; 8], modulus: &[u32; 8]) {
    let modulus = u32_array_to_biguint_le(modulus);
    let value = (u32_array_to_biguint_le(x) * u32_array_to_biguint_le(y)) % modulus;
    *result = biguint_to_u32_array_le(&value);
}
...
pub(crate) fn mont_mul_zkvm(result: &mut [u64; 4], x: &[u64; 4], y: &[u64; 4]) {
    ...
    backend_mul_mod_u32(&mut z, &x_32, &y_32, &fq);
    backend_mul_mod_u32(&mut out, &z, &inv_r, &fq);
...
pub(crate) fn mod_mul_by5_zkvm(result: &mut [u64; 4]) {
    ...
    let mut out: [u32; BIGINT_WIDTH_WORDS] = [0; BIGINT_WIDTH_WORDS];
    backend_mul_mod_u32(&mut out, &x_32, &e5, &fq);
    u32_array_to_u64_array_le(&out, result);
}
...
pub(crate) fn mod_mul_by5_u32_zkvm(result: &mut [u32; 8]) {
    ...
    backend_mul_mod_u32(&mut temp_result, result, &e5, &FQ);
    *result = temp_result;
}
...
pub(crate) fn mod_mul_zkvm(result: &mut [u64; 4], x: &[u64; 4], y: &[u64; 4]) {
    ...
    backend_mul_mod_u32(&mut out, &x_32, &y_32, &fq);
    u32_array_to_u64_array_le(&out, result);
}

pub(crate) fn mod_mul_u32_zkvm(result: &mut [u32; 8], x: &[u32; 8], y: &[u32; 8]) {
    backend_mul_mod_u32(result, x, y, &FQ);
}
/*
...
pub(crate) fn from_mont_to_bigint(result: &mut [u64; 4], x: &[u64; 4]) {
    ...
    backend_mul_mod_u32(&mut out, &x_32, &inv_r, &fq);
    u32_array_to_u64_array_le(&out, result);
}
...
pub(crate) fn from_bigint_to_mont(result: &mut [u64; 4], x: &[u64; 4]) {
    ...
    backend_mul_mod_u32(&mut out, &x_32, &r, &fq);
    u32_array_to_u64_array_le(&out, result);
}
...
pub(crate) fn project_from_bigint_to_mont(
    ...
    backend_mul_mod_u32(&mut t_x, &x_32, &r, &fq);
    backend_mul_mod_u32(&mut t_y, &y_32, &r, &fq);
    backend_mul_mod_u32(&mut t_t, &t_32, &r, &fq);
    backend_mul_mod_u32(&mut t_z, &z_32, &r, &fq);
...
mod tests {
    ...
    #[test]
    fn add_projective_matches_reference_after_roundtrip() {
        let generator = Element::prime_subgroup_generator();
        let lhs = (generator * Fr::from(3u64)).0;
        let rhs = (generator * Fr::from(5u64)).0;

        let expected = lhs + rhs;

        let mut got = projective_mont_to_bigint(&lhs);
        let rhs_bigint = projective_mont_to_bigint(&rhs);
        add_projective(&mut got, &rhs_bigint);
        let got = projective_bigint_to_mont(&got);
