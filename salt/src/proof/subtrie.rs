...
//! 7. Optionally advance the process-wide node-polynomial cache to the witnessed block's
...
fn apply_refresh(
    ...
    for (physical, patch) in plan {
        let base = match in_hand.get(&physical) {
            ...
            None => match node_poly_cache::CACHE.get(physical, &patch.old) {
                Some(poly) => poly,
                None => continue,
            },
        };
        ...
        node_poly_cache::CACHE.insert(physical, patch.new, Arc::new(LagrangeBasis::new(values)));
        refreshed += 1;
    }
    refreshed
}
    ...
    Store: StateReader + TrieReader,
{
    ...
    // Step 6: One IPA prover query per proven point, in node order. With a refresh, every
    // resolved polynomial is also kept by physical parent id with the commitment it was read
    // at: the witness reads the pre-state, so these are the bases the block's transition
    // transforms.
    ...
    for ((parent, points), poly) in parents.iter().zip(points).zip(polys) {
        if refresh.is_some() {
            in_hand.insert(parent.physical, (parent.bytes, Arc::clone(&poly)));
        }
...
mod tests {
    ...
    fn rebuilt_polynomial(store: &MemStore, node: NodeId) -> LagrangeBasis {
        if is_leaf_node(node) {
            ...
            internal_polynomials(store, &[node]).unwrap().remove(0)
        }
    }
    ...
    fn refreshed(node: NodeId, new: &CommitmentBytes) -> Option<Arc<LagrangeBasis>> {
        let entry = node_poly_cache::CACHE.get(node, new);
    ...
    fn refresh_skips_buckets_whose_capacity_changed() {
        ...
        let run = |updates: StateUpdates, witnessed: &[SaltKey]| {
            ...
            assert!(
                node_poly_cache::CACHE.get(root, &new_root).is_none(),
                "the frozen bucket's root was refreshed"
            );
            for &(node, (_, new)) in &trie_updates {
                if node >> BUCKET_SLOT_BITS != 0 {
                    assert!(
                        node_poly_cache::CACHE.get(node, &new).is_none(),
    ...
    fn refresh_uses_cached_bases_for_parents_the_witness_did_not_read() {
        ...
        store.update_trie(trie_b.clone());
        for &(node, new) in &unread_roots {
            let entry = node_poly_cache::CACHE
                .get(node, &new)
                .unwrap_or_else(|| panic!("node {node} was not refreshed from its cached base"));
            assert_eq!(*entry, rebuilt_polynomial(&store, node), "node {node}");
        }
        for &(node, (_, new)) in &trie_b {
            if let Some(entry) = node_poly_cache::CACHE.get(node, &new) {
                assert_eq!(*entry, rebuilt_polynomial(&store, node), "node {node}");
            }
        }
    }
    ...
    fn refresh_patches_a_rehashed_bucket_with_unchanged_capacity() {
        ...
        if let Some(entry) = node_poly_cache::CACHE.get(meta_root, &new_meta_root) {
            assert_eq!(
                *entry,
                rebuilt_polynomial(&store, meta_root),
                "metadata leaf"
